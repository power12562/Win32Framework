...
public:
	...
	Clip AddClip(const WCHAR* fileName, bool IsLoop = true); //애니메이션 클립 추가
	void SetClip(My::List<ClipClass>::Node* clip); //재생할 클립 선택
	void DelClip(My::List<ClipClass>::Node* clip); //클립 삭제
...
protected:
	My::List<ClipClass> ClipList; //클립 리스트
	ClipClass* CurrentClip = nullptr; //현재 선택된 클립
